...
const GUIDES_DIR = path.resolve(OUTPUT_DIR)
...
downloader.fetchFiles('pansarhq', 'krans', 'guides').then(async (response) => {
    const files = response.map(({ path, contents }) => ({
        path,
        content: contents.toString('utf-8'),
    }))
    for (const file of files) {
        console.log(`Generating guide ${file.path}`)
        fs.outputFileSync(path.resolve(GUIDES_DIR, file.path), file.content)
    }
}, console.error)
